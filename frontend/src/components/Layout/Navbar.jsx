...
export default function Navbar() {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 bg-white/95 dark:bg-[#0B0F19]/95 backdrop-blur-sm border-b border-slate-200/80 dark:border-slate-800 transition-colors duration-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          
          {/* Brand Logo */}
          <Link to="/" className="flex items-center gap-2.5 group">
            <div className="w-8 h-8 rounded-lg bg-[#0F172A] dark:bg-slate-800 border border-transparent dark:border-slate-700 flex items-center justify-center text-white font-bold text-xs shadow-sm group-hover:scale-105 transition-transform">
              <span className="text-orange-500 font-extrabold">J</span>
              <span className="text-white font-extrabold">S</span>
            </div>
            <div className="flex flex-col">
              <span className="text-[15px] font-bold text-[#0F172A] dark:text-white leading-none tracking-tight">JanSetu</span>
              <span className="text-[9px] font-semibold text-slate-400 dark:text-slate-400 tracking-wider uppercase mt-0.5">AI Assistant</span>
            </div>
          </Link>
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `text-[13px] font-semibold transition-colors pb-1 ${
                  isActive
                    ? 'text-[#0F172A] dark:text-white border-b-2 border-[#0F172A] dark:border-white'
            ...
            <div className="relative" ref={langRef}>
              ...
              {langDropdownOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-[#131B2E] rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 py-1.5 z-50 animate-in fade-in slide-in-from-top-1">
                  <div className="px-3.5 py-1.5 text-[10px] font-bold text-slate-400 dark:text-slate-400 uppercase tracking-wider border-b border-slate-100 dark:border-slate-800 mb-1">
                    Select Language
                  </div>
                  {languages.map((lang) => (
                    <button
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-2 sm:gap-2.5">
                ...
                <Link
                  ...
                  className="flex items-center gap-1.5 text-[12px] font-semibold text-amber-900 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/40 hover:bg-amber-100/90 dark:hover:bg-amber-900/50 border border-amber-200/80 dark:border-amber-800/50 px-2.5 sm:px-3 py-1.5 rounded-xl transition-all shadow-2xs cursor-pointer"
                >
                  ...
                  {savedSchemes.length > 0 && (
                    <span className="text-[10px] font-extrabold px-1.5 py-0.5 rounded-full bg-amber-200 dark:bg-amber-800 text-amber-900 dark:text-amber-100 leading-none">
                      {savedSchemes.length}
                    </span>
                  )}
                </Link>
                ...
                <Link
                  to="/assistant"
                  className="flex items-center gap-1.5 text-[12px] font-semibold text-white bg-gradient-to-r from-[#F97316] to-[#EA580C] px-3.5 py-1.5 rounded-xl hover:from-[#EA580C] hover:to-[#C2410C] transition-all shadow-sm cursor-pointer"
                >
                ...
                <div className="relative" ref={profileRef}>
                  <button
                    ...
                    className="w-9 h-9 rounded-full bg-[#0A1633] dark:bg-slate-800 hover:bg-slate-900 dark:hover:bg-slate-700 flex items-center justify-center text-white text-xs font-extrabold shadow-sm transition-all cursor-pointer ring-2 ring-transparent hover:ring-orange-400 border border-transparent dark:border-slate-700"
                  ...
                  {/* Profile Popup Menu */}
                  {profileDropdownOpen && (
                    <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-[#131B2E] rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 p-2 z-50 animate-in fade-in slide-in-from-top-2">
                      ...
                      <div className="py-1.5 flex flex-col gap-0.5">
                        <Link
                          ...
                          className="flex items-center gap-2.5 px-3 py-2 text-xs font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white rounded-xl transition-colors"
                        >
                      ...
                      <div className="pt-1 border-t border-slate-100 dark:border-slate-800">
                        <button
                          ...
                          className="w-full flex items-center gap-2.5 px-3 py-2 text-xs font-bold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/40 rounded-xl transition-colors cursor-pointer"
                        >
              ...
              <div className="flex items-center gap-2">
                <Link
                  to="/login"
                  className="text-[13px] font-semibold text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white px-3 py-1.5 rounded-xl transition-colors cursor-pointer"
                >
                  {nav.signIn}
                </Link>
                <Link
                  to="/register"
                  className="text-[12px] font-bold text-white bg-[#0F172A] dark:bg-orange-600 hover:bg-slate-800 dark:hover:bg-orange-500 px-3.5 py-1.5 rounded-xl transition-colors shadow-sm cursor-pointer"
                >
                  {nav.register}
                </Link>
              </div>
